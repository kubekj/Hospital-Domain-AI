...
from src.cbs.npaction import NPActionType, NPAction
...
class NPState:
    ...
    def result(self, joint_action: "[NPAction, ...]") -> "NPState":
        """
        ...
        for agent, action in enumerate(joint_action):
            ...
            if action.action_type == NPActionType.NoOp:
                continue

            if action.action_type in [NPActionType.Move, NPActionType.Push, NPActionType.Pull]:
                copy_agent_rows[agent] += delta_row
                copy_agent_cols[agent] += delta_col

            if action.action_type in [NPActionType.Push, NPActionType.Pull]:
                if action.type == NPActionType.Push:
        ...
        copy_state = NPState(copy_agent_rows, copy_agent_cols, copy_boxes)
    ...
    def is_goal_state(self) -> bool:
        ...
        """
        for (row, col), goal in np.ndenumerate(NPState.goals):
            if 'A' <= goal <= 'Z':
    ...
    def get_expanded_states(self) -> "[NPState, ...]":
        ...
        applicable_actions = [
            [action for action in NPAction if self.is_applicable(agent, action)] for agent in range(num_agents)
        ]
        ...
        NPState._RNG.shuffle(expanded_states)

        return expanded_states

    def is_applicable(self, agent: int, action: "NPAction") -> bool:
        ...
        if action.action_type == NPActionType.Move:
            return self.is_free(destination_row, destination_col)

        elif action.action_type == NPActionType.Push:
            ...
            if box:  # Check if box exists at the location
                ...
                return (NPState.box_colors[ord(box) - ord('A')] == NPState.agent_colors[agent] and
                        self.is_free(next_row, next_col))

        elif action.action_type == NPActionType.Pull:
            ...
            if box and self.is_free(destination_row, destination_col):  # Check if box exists and destination is free
                return NPState.box_colors[ord(box) - ord('A')] == NPState.agent_colors[agent]
    ...
    def is_conflicting(self, joint_action: "[NPAction, ...]") -> "bool":
        ...
        for agent in range(num_agents):
            ...
            elif action.action_type == NPActionType.Move:
        ...
        for a1 in range(num_agents):
            if joint_action[a1] is NPAction.NoOp:
                continue

            for a2 in range(a1 + 1, num_agents):
                if joint_action[a2] is NPAction.NoOp:
                    continue
                ...
                if joint_action[a1].action_type == NPActionType.Push:
                    if (box_rows[a1] + joint_action[a1].box_row_delta == destination_rows[a2]
                        ...
                        return True
                if joint_action[a2].action_type == NPActionType.Push:
    ...
    def is_free(self, row: "int", col: "int") -> "bool":
        ...
        """
        return not NPState.walls[row][col] and self.boxes[row][col] == "" and self.agent_at(row, col) is None
    ...
    def __hash__(self):
        if self._hash is None:
            ...
            _hash = _hash * prime + hash(tuple(NPState.agent_colors))
            _hash = _hash * prime + hash(tuple(tuple(row) for row in self.boxes))
            _hash = _hash * prime + hash(tuple(NPState.box_colors))
            _hash = _hash * prime + hash(tuple(tuple(row) for row in NPState.goals))
            _hash = _hash * prime + hash(tuple(tuple(row) for row in NPState.walls))
            self._hash = _hash
        return self._hash
    ...
    def __eq__(self, other):
        if self is other:
            return True
        if not isinstance(other, NPState):
            return False
        if self.agent_rows != other.agent_rows:
            return False
        if self.agent_cols != other.agent_cols:
            return False
        if NPState.agent_colors != other.agent_colors:
            return False
        if NPState.walls != other.walls:
            return False
        if self.boxes != other.boxes:
            return False
        if NPState.box_colors != other.box_colors:
            return False
        if NPState.goals != other.goals:
            return False
        return True
    ...
    def __repr__(self):
        ...
        for row in range(len(self.boxes)):
            ...
            for col in range(len(self.boxes[row])):
                ...
                elif NPState.walls[row][col] is not None:
                    line.append("+")
